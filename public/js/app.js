...
const App = {
    ...
    render(page) {
        ...
        switch(page) {
            case 'dashboard':
                ...
                break;
            case 'api':
                content = this.createAPIPage();
                break;
            case 'department':
                content = this.createDepartmentPage();
                break;
            case 'behavior':
    ...
    createChartCard(title, chartId, chartType) {
        return `
    ...
    createDashboardPage() {
        const kpis = MockData.getKPIData().dashboard;
        ...
        return `
            ...
            <div class="bento-grid">
                ${this.createChartCard('Monthly Usage Trends', 'monthlyChart', 'bar')}
                <div class="col-span-6 glass-panel rounded-xl p-lg">
                    <div class="flex justify-between items-center mb-md">
                    ...
                    <div class="flex flex-wrap gap-md" id="trending-topics">
                        ${MockData.getTrendingTopics().map(t => `
    ...
    createAPIPage() {
        const kpis = MockData.getKPIData().api;
        const modelData = MockData.getModelTokenConsumption();
        const hierarchyData = MockData.getHierarchyData();
                ...
                <div class="col-span-6 glass-panel rounded-xl p-lg">
                    ...
                    <div class="space-y-md">
                        <div class="flex justify-between items-center pb-md border-b border-outline-variant">
                            ...
                            <span class="font-headline-md text-headline-md text-on-surface font-bold">${kpis[0].value}</span>
                        </div>
                        <div class="flex justify-between items-center pb-md border-b border-outline-variant">
                            <span class="font-body-md text-body-md text-on-surface-variant">Projected End of Month</span>
                            <span class="font-headline-md text-headline-md text-error font-bold">${kpis[1].value}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="font-body-md text-body-md text-on-surface-variant">Cost Efficiency</span>
                            <span class="font-headline-md text-headline-md text-primary font-bold">${kpis[2].value}</span>
                        </div>
                        <div class="mt-md p-md bg-primary/10 border border-primary rounded-lg">
                            <p class="font-body-md text-body-md text-primary">✓ Caching saved ~8% this period.</p>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <tbody>
                            ${hierarchyData.map(row => `
                                <tr class="border-b border-surface-container hover:bg-surface-container-low transition-colors">
                                    ...
                                    <td class="py-md px-md text-body-md text-on-surface font-semibold">${(row.tokens / 1000000).toFixed(1)}M</td>
                                </tr>
                            `).join('')}
                        </tbody>
                    </table>
    ...
    createDepartmentPage() {
        const kpis = MockData.getKPIData().department;
        const departmentData = MockData.getDepartmentData();
        ...
        return `
            ...
            <div class="bento-grid mb-gutter">
                ${this.createChartCard('Monthly Growth Rates by Dept', 'growthChart', 'bar')}
                <div class="col-span-6 glass-panel rounded-xl p-lg">
                    <div class="flex justify-between items-center mb-md">
                        ...
                        <tbody>
                            ${departmentData.map(dept => `
                                <tr class="border-b border-surface-container hover:bg-surface-container-low transition-colors">
                                    ...
                                    <td class="py-md px-md text-body-md text-on-surface">${dept.models.toLocaleString()}</td>
                                    ...
                                    <td class="py-md px-md">
                                        <span class="px-md py-xs rounded-full font-label-md text-label-md ${dept.statusColor} text-on-primary text-xs">
                                            ● ${dept.status}
                                        </span>
                                    </td>
                                </tr>
    ...
    createBehaviorPage() {
        const kpis = MockData.getKPIData().behavior;
        const dailyUsers = MockData.getDailyActiveUsers();
        const appDist = MockData.getTopAppDistribution();
        const tags = MockData.getPopularTags();
        ...
        return `
            ...
            <div class="bento-grid mb-gutter">
                <div class="col-span-3 glass-panel rounded-xl p-lg">
                    ...
                    <div class="font-display-lg text-display-lg text-on-surface mb-md">${kpis[0].value}</div>
                    <div class="font-label-md text-label-md text-primary flex items-center gap-1">
                        <span class="material-symbols-outlined text-[14px]">trending_up</span> ${kpis[0].change}
                    </div>
                </div>
            ...
            <div class="bento-grid">
                ${this.createChartCard('Daily Active Users', 'dailyChart', 'line')}
                <div class="col-span-6 glass-panel rounded-xl p-lg">
                    <div class="flex justify-between items-center mb-md">
                        ...
                        ${appDist.map((app, idx) => `
                            <div>
                                <div class="flex justify-between mb-xs">
                                    <span class="font-body-md text-body-md text-on-surface">${app.app}</span>
                                    <span class="font-label-md text-label-md text-on-surface-variant">${app.percentage}%</span>
                                </div>
                                <div class="h-2 bg-surface-container rounded-full overflow-hidden">
                                    <div class="h-full" style="background-color: ${app.color}; width: ${app.percentage}%"></div>
                                </div>
                            </div>
                        `).join('')}
                    </div>
